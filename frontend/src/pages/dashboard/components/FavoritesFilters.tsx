...
export function FavoritesFilters({
  ...
  return (
    <div className={`flex flex-col gap-3 sm:flex-row sm:items-end ${styles.card} p-4`}>
      <FormField id="dash-search" label="Buscar por nombre" className="flex-1">
        <div className="relative">
          ...
          <TextInput id="dash-search" className="pl-10" value={search} onChange={(e) => onSearchChange(e.target.value)} placeholder="pikachu…" />
        </div>
      </FormField>
      <FormField id="dash-type" label="Tipo" className="w-full sm:w-40">
        <TextInput
          id="dash-type"
          value={typeFilter}
          onChange={(e) => onTypeFilterChange(e.target.value.toLowerCase().replace(/\s/g, ''))}
